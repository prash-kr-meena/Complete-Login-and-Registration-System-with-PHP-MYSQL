...
if ( isset($_POST['sbt']) ) {  //ie. if the password reset form is submitted then first validate this form and then if its valid process the form.
	...
	$id = $id_array[1]; #--> in case if we try to give the id explicitly by putting in the url and if no such delimiter existed in it then the index will be undefined..
						#  but for the case now let it be 
	# echo $id;
	
}else{
	 echo Not_authorized('You are not authorized to view this page.', 'index.php','Back'); 
}
